...
**Assignment `newMessage` / attempt #1:** set `INSPECTIO_WORKER_ACTIVATION_URLS` to a comma-separated list of worker base URLs (e.g. `http://127.0.0.1:8004`). After each durable pending write, the API schedules a **background** `POST` to `/internal/v1/activate-pending` (or **`/internal/v1/activate-pending-batch`** for `POST /messages/repeat`) on the URL selected by `shard_id // INSPECTIO_WORKER_SHARDS_PER_POD` (default matches `SHARDS_PER_POD`). The worker **enqueues** the row and **wakes** its scheduler so attempt #1 runs on the next `run_tick` without blocking the HTTP response. If activation URLs are empty or the call fails, due work is still picked up on the next **500ms** tick (degraded). **`INSPECTIO_WORKER_ACTIVATION_BATCH_MAX_KEYS`** (default `512`) caps keys per batch request (must match worker validation). The PDF’s `send` / `newMessage` / `wakeup` names are documented on `AssignmentSchedulerHooks` in `inspectio_exercise.domain.assignment_scheduler_surface`.
