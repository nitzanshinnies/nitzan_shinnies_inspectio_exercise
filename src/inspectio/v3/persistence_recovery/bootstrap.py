...
class PersistenceRecoveryBootstrap:
    ...
    async def recover(self) -> RecoverySnapshot:
        ...
        for shard in self._shard_ids:
            _ = await self._load_checkpoint(shard=shard)
            for key in await self._segment_keys(shard=shard):
                loaded_segments += 1
                segment_events = await self._read_segment_events(key=key)
                loaded_events += len(segment_events)
                for event in sorted_for_replay(segment_events):
    ...
    async def _segment_keys(self, *, shard: int) -> list[str]:
        prefix = _segment_prefix(shard=shard)
        return sorted(await self._store.list_keys(prefix=prefix))
