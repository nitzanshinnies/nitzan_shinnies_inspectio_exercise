...
| Step | Task | Detail |
|------|------|--------|
| 1.1 | **Confirm ConfigMap key present** | `kubectl -n inspectio get cm inspectio-v3-config -o jsonpath='{.data.INSPECTIO_V3_PERSISTENCE_ACK_DELETE_MAX_CONCURRENCY}'` — if empty, **add** key (default in repo template is **`"6"`** in `deploy/kubernetes/configmap.yaml`). |
| 1.2 | **Raise ack delete concurrency in steps** | Increase **`INSPECTIO_V3_PERSISTENCE_ACK_DELETE_MAX_CONCURRENCY`** within **1..8** (clamped in `sqs_consumer.py`). Baseline default is **`6`**; suggested sweep on EKS if still ack-bound: **`6 → 8`** (or **`4 → 6 → 8`** from a conservative cluster) with one **Plan D** **`iter-N`** per value. |
