...
| **LocalStack init** | **`deploy/localstack/init/ready.d/10-inspectio-aws.sh`** | Creates S3 bucket **`inspectio-dev`** and Kinesis stream **`inspectio-ingest`** (`INSPECTIO_KINESIS_LOCAL_SHARDS`, default **1**) |
...
**Target compose env (when app services exist):** `AWS_ENDPOINT_URL=http://localstack:4566`, **`INSPECTIO_S3_BUCKET=inspectio-dev`**, **`INSPECTIO_KINESIS_STREAM_NAME=inspectio-ingest`**, **`INSPECTIO_SMS_URL=http://mock-sms:8080`**, **`INSPECTIO_NOTIFICATION_BASE_URL=http://inspectio-notification:8081`**, **`INSPECTIO_REDIS_URL=redis://redis:6379/0`**.
